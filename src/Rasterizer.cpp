...
Rasterizer::Rasterizer(int width, int height) :
    width(width), height(height), frame_buffer(width * height, {0.f, 0.f, 0.f}), z_buffer(width * height, std::numeric_limits<float>::max())
{
}
...
void Rasterizer::drawTriangle(const triangle_t& triangle, ShaderInfo& shader_info)
{
	...
	int min_x = std::min({triangle.vertices[0].x(), triangle.vertices[1].x(), triangle.vertices[2].x()});
	int min_y = std::min({triangle.vertices[0].y(), triangle.vertices[1].y(), triangle.vertices[2].y()});
	int max_x = std::max({triangle.vertices[0].x(), triangle.vertices[1].x(), triangle.vertices[2].x()}) + 1;
	int max_y = std::max({triangle.vertices[0].y(), triangle.vertices[1].y(), triangle.vertices[2].y()}) + 1;
...
bool Rasterizer::isInsideTriangle(int x, int y, const triangle_t& triangle)
{
